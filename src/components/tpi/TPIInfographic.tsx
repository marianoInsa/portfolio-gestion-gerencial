...
// FÁCILMENTE REVERSIBLE: Si se define en false, la infografía es estática y sin lightbox.
const ENABLE_LIGHTBOX = true;

interface TPIInfographicProps {
  src: string;
  alt: string;
}

export default function TPIInfographic({ src, alt }: TPIInfographicProps) {
  ...
  return (
    <div className="w-full">
      ...
      <div
        onClick={handleOpen}
        className={`group relative overflow-hidden rounded-2xl border border-nebula bg-dark-matter/30 p-2 backdrop-blur-sm transition-all duration-300 ${
          ENABLE_LIGHTBOX ? 'cursor-pointer hover:border-cyber-cyan/70 hover:shadow-[0_0_25px_rgba(0,245,255,0.15)]' : ''
        }`}
      >
        <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-void-black">
          <img
            src={src}
            alt={alt}
          ...
          {ENABLE_LIGHTBOX && (
            <div className="absolute inset-0 flex items-center justify-center bg-void-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              <div className="flex items-center gap-2 rounded-xl bg-deep-space/90 border border-cyber-cyan/50 px-4 py-2.5 text-sm font-semibold text-cyber-cyan shadow-[0_0_20px_rgba(0,245,255,0.3)]">
                <Maximize2 size={16} />
                Expandir Infografía
              </div>
            </div>
      ...
      <p className="mt-3 text-center text-xs text-star-light">
        Visualización sintetizada del Trabajo Práctico Integrador: del diagnóstico a la transformación organizativa de ECOM Chaco S.A.
      </p>

      {/* Lightbox / Modal de Zoom */}
      {ENABLE_LIGHTBOX && isOpen && (
        <div
        ...
        >
          <div className="relative flex h-full w-full max-w-7xl flex-col items-center justify-center">
            ...
            <div className="mt-4 text-center">
              <p className="font-exo2 text-sm font-semibold text-white-photon">{alt}</p>
